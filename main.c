...
{	
	...
	while(SWITCH_S1)
		...
		} else
		{
			...
			for (n = FILTER_SIZE - 1; n > 0; n--) 
				signalBuffer[n] = signalBuffer[n - 1];

			// Put the next sample in the buffer.
			signalBuffer[0] = rxSignal;

			// Calculate the dot product.
			for (n = 0; n < FILTER_SIZE; n++)
				filterSum += ((long signed int)signalBuffer[n] * 
								(long signed int)coefficents[n]) << 1;	

			// Put the result in Q15 format.
			txSignal = filterSum >> 16;
